...
export default class RouterBuild {
        ...
        this.#imports('middlewares', (err, middlewaresImported) => {
            ...
            this.#imports('managers', (err, mapManagers) => {
                ...
                Object.keys(mapManagers).forEach((router) => {
                    ...
                    Router.all('*', ...this.#buildMiddlewares(router));
    ...
    #buildMiddlewares(route, manager = false){
        ...
        if(!manager) middlewares.push(...this.#middlewaresConfig['*'])
        else instanceName = manager.constructor.name.toLowerCase();
        if (this.#middlewaresConfig[route]) middlewares.push(...this.#middlewaresConfig[route][instanceName || '*']);
    ...
    #isValidManager(manager, router) {
        if (!manager.method.length || !manager.url || !manager.handler) {
            try {
